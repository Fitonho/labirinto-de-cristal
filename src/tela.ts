        ...
        for (let i = 0; i < this.rows.length; i++) {
            for (let j = 0; j < this.rows[i].children.length; j++) {
                ...
                if (this.matriz[i][j].event == BlockEvent.endOfLevel) {
                    (<any>document.styleSheets[1]).cssRules[3].style.borderTopColor = this.invertColor(this.matriz[i][j].color);
                    (<any>document.styleSheets[1]).cssRules[2].style.borderBottomColor = this.invertColor(this.matriz[i][j].color);
